...
class DbRepository:
    ...
    def get_data_for_time_range(self, start_time: str, end_time: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        with self.engine.connect() as conn:
            df_fpn = pd.read_sql(raw_query("fpn"), conn, params=(start_time, end_time), index_col="unit",
                                 parse_dates=['timeFrom', 'timeTo'])
            df_boal = pd.read_sql(raw_query("boal"), conn, params=(start_time, end_time), index_col="unit",
                                  parse_dates=['timeFrom', 'timeTo'])
